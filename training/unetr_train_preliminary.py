"""
Training script — 3D UNETR, preliminary experiment (200 epochs).
GPU-ready (falls back to CPU automatically).

This is the first version of the UNETR experiment, run originally on a
GPU server. It uses a smaller architecture (feature_size=24) and standard
Adam optimiser. The final model used in the paper is in unetr_train.py.

Architecture
------------
- UNETR (MONAI), feature_size=24, hidden_size=384, mlp_dim=1536, num_heads=6
...
def build_training_components(device, fold):
    ...
    print(f"[FOLD {fold}] Model: UNETR 3D | feature_size=24 | hidden_size=384 | "
          f"mlp_dim=1536 | num_heads=6 | dropout=0.1")
...
# ============================================================
# POST-TRAINING EVALUATION (per-case, internal val + test)
# ============================================================
